(() => {
    ...
    const SEL = {
      container: "#coursesContainer, [data-courses-container]",
    ...
    const loadInitialCourses = () => {
      ...
      if (scriptJSON) {
        try { const data = JSON.parse(scriptJSON.textContent || "[]"); if (Array.isArray(data)) return data; } catch (_) {}
      }
      return [];
    };
    const state = {
    ...
    function getFilterValues() {
      ...
      return {
        ...
        tipos: tipoSelecionados,
        carga: (state.els.carga && state.els.carga.value) || "",
        nivel: (nivelSel && nivelSel.value) || "",
    ...
    function updateUI() { renderCourses(); updateCourseButtons(); }
    function renderCourses() {
      ...
      if (!list.length) {
        wrap.innerHTML = `<div class="ib-empty"><div class="ib-empty__icon">📚</div><p class="ib-empty__text">Nenhum curso encontrado.</p></div>`;
        return;
      }
    ...
    function renderCourseCard(c) {
      ...
      const thumb = c.thumbnail ? `<img class="ib-card__thumb" src="${c.thumbnail}" alt="${escapeHtml(c.title)}" loading="lazy">` : "";
      card.innerHTML = `<div class="ib-card__media">${thumb}</div><div class="ib-card__body"><h3 class="ib-card__title">${escapeHtml(c.title)}</h3><p class="ib-card__desc">${escapeHtml(c.description || "")}</p></div><div class="ib-card__actions">${renderPrimaryButton(c)}<button class="ib-btn ib-btn--ghost" data-action="details" data-id="${c.id}">Ver detalhes</button></div>`;
    ...
    function renderPrimaryButton(c) {
      ...
      return enrolled ? `<button class="ib-btn ib-btn--primary" data-action="access" data-id="${c.id}">Acessar curso</button>` : `<button class="ib-btn ib-btn--primary" data-action="enroll" data-id="${c.id}">Inscrever-se</button>`;
    }
    ...
    function enrollCourse(course) {
      ...
      } else { state.userEnrollments.add(String(course.id)); }
      updateCourseButtons();
    }

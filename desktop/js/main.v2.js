// Instituto Buriti — main.v2.js (CORRIGIDO - rotas /desktop/pages/)
// Objetivo: padronizar rotas absolutas para /desktop/pages/, corrigir qualquer referência
// "detalhes-cursos.html" (plural) para "/desktop/pages/detalhes-curso.html" (singular),
...
(function hardenDetailsRoute() {
  ...
  try {
    const DETAILS_PAGE = '/desktop/pages/detalhes-curso.html';
...
// ---- Constantes centrais de rota (CORRIGIDAS) ----
const DETAILS_PAGE = '/desktop/pages/detalhes-curso.html';
...
document.addEventListener('DOMContentLoaded', function () {
  normalizeAllLinksToAbsoluteDesktopPages();
  ...
  console.log('main.v2.js: inicializado com rotas absolutas /desktop/pages/ e guardas de detalhes.');
});

// --- 1) Normalização global de links /desktop/pages/... + correção singular ---
function normalizeAllLinksToAbsoluteDesktopPages() {
  ...
  aTags.forEach(a => {
    ...
    // (B) Qualquer variação que contenha /pages/ vira /desktop/pages/
    const pagesMatch = raw.match(/(^|\/)pages\/(.+)/i);
    if (pagesMatch) {
      a.setAttribute('href', '/desktop/pages/' + pagesMatch[2].replace(/^\/+/, ''));
      return;
    }

    // (C) Se o link é apenas "arquivo.html", transforme em "/desktop/pages/arquivo.html"
    if (/^[a-z0-9\-]+\.html$/i.test(raw)) {
      a.setAttribute('href', '/desktop/pages/' + raw);
      return;
    }

    // (D) Home raiz deve ser absoluta para /desktop/
    if (raw === 'index.html' || raw === './index.html' || raw === '../index.html') {
      a.setAttribute('href', '/desktop/index.html');
      return;
    }
  });
}
...
function rewriteCourseDetailAnchors() {
  ...
  anchors.forEach(a => {
    ...
    // Garante prefixo absoluto /desktop/pages/
...
// --- Navegação Principal (CORRIGIDA para /desktop/pages/) ---
function initializeMainNavigation() {
  ...
  specificButtons.forEach(button => {
    ...
    if (/cursos/.test(normalized) && !/detalhes-curso/.test(normalized)) {
      button.setAttribute('href', '/desktop/pages/cursos.html');
    } else if (/quem-somos/.test(normalized)) {
      button.setAttribute('href', '/desktop/pages/quem-somos.html');
    } else if (/seja-?instrutor/.test(normalized)) {
      button.setAttribute('href', '/desktop/pages/seja-instrutor-novo.html');
    } else if (/contato/.test(normalized)) {
      button.setAttribute('href', '/desktop/pages/contato.html');
  ...
  console.log('Navegação principal inicializada (rotas absolutas /desktop/pages/...).');
}
